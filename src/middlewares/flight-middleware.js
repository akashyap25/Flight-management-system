...
function validateCreateRequest(req, res, next) {
    ...
    if (!req.body.price) {
        ErrorResponse.message = 'Something went wrong while creating flight';
        ErrorResponse.error = new AppError(['Price Number not found in the incoming request in the correct form'], StatusCodes.BAD_REQUEST);
        return res
            .status(StatusCodes.BAD_REQUEST)
            .json(ErrorResponse);
    }
...
function validateUpdateSeatsRequest(req,res,next){
    if (!req.body.seats) {
        ErrorResponse.message = 'Something went wrong while creating updating Seats';
